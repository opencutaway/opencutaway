...
Status words. *Playable* means the code, content, JSON Schema, and Playwright proof have landed in lockstep; the generated feature map's `shipped` status is that same lockstep label. A sitting is *shipped* in this table only when it is playable and its kid-facing copy has passed the human ROLE-EDITOR gate, recorded under `docs/reviews/`. Art stays placeholder until `F-art-geometry-migration` closes.
